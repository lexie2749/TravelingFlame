%% Unified PINN-CFD Hybrid for Two Colliding Ring Flames
% This script runs a hybrid PINN-CFD simulation for two reacting ring
% flame fronts that propagate and collide. It computes the flame speed as a
% function of the ring's inner radius.
% It is a unified version based on the logic from untitled7.m and untitled4.m.
clear; close all; clc;

% --- Main Execution Loop ---
% =========================================================================
% Set up the radius range and pre-allocate storage
R_inner_start = 0.4; % 25 mm inner radius
R_inner_end = 0.5;   % 75 mm inner radius
...
% Start the loop to run simulations for each inner radius
for i = 1:numRadii
    R_inner = radii_vec(i);
    ...
    %% 1. Create Grid and Initial Condition
    % =====================================================================
    % Grid setup in polar coordinates
    ...
    % Initialize temperature and fuel fields
    T = zeros(Nr, Ntheta, Nt);
    Y_fuel = ones(Nr, Ntheta, Nt);
    
    % Create TWO initial flame fronts at opposite ends of the ring
    theta_ignition1 = pi/4;
    theta_ignition2 = theta_ignition1 + pi;
    for i_r = 1:Nr
        for j_th = 1:Ntheta
            % First flame front
            T(i_r, j_th, 1) = T(i_r, j_th, 1) + 0.1 + 0.9 * exp(-((theta(j_th) - theta_ignition1) / 0.3)^2);
            Y_fuel(i_r, j_th, 1) = Y_fuel(i_r, j_th, 1) - 0.5 * exp(-((theta(j_th) - theta_ignition1) / 0.3)^2);
            
            % Second flame front on the opposite side
            T(i_r, j_th, 1) = T(i_r, j_th, 1) + 0.1 + 0.9 * exp(-((theta(j_th) - theta_ignition2) / 0.3)^2);
            Y_fuel(i_r, j_th, 1) = Y_fuel(i_r, j_th, 1) - 0.5 * exp(-((theta(j_th) - theta_ignition2) / 0.3)^2);
        end
    end
    
    %% 2. Create Neural Network and Training Data
    % =====================================================================
    % Build network layers (as in untitled7.m)
    layers = [
    ...
    for l = 1:numTrainPoints
        phase1 = theta_train(l) - wave_speed * t_train(l);
        phase2 = theta_train(l) - (pi + wave_speed * t_train(l));
        
        T_target_1 = 0.5 * (1 + tanh(5 * (cos(phase1) - 0.5)));
        Y_target_fuel_1 = 1 - 0.5 * T_target_1;
        
        T_target_2 = 0.5 * (1 + tanh(5 * (cos(phase2) - 0.5)));
        Y_target_fuel_2 = 1 - 0.5 * T_target_2;
        
        T_target(l) = max(T_target_1, T_target_2);
        Y_target_fuel(l) = min(Y_target_fuel_1, Y_target_fuel_2);
    end
    ...
    %% 4. Hybrid CFD-PINN Simulation
    % =====================================================================
    fprintf('Running hybrid simulation for R_inner = %.3f m...\n', R_inner);
    tic;
    for n = 2:Nt
        current_t = (n-1) * dt;
        ...
        for i_r = 2:Nr-1
            for j_th = 1:Ntheta
                if flame_front(i_r, j_th)
                    ...
                    % Use finite difference in other regions
    ...
    fprintf('Simulation for R_inner = %.3f m complete. Flame speed: %.3f rad/s\n\n', R_inner, flame_speed);
    
end % End of the main loop
%% 5. Final Plots
% =========================================================================
% Plot 1: Flame speed vs. radius
figure;
plot(radii_vec * 1000, flame_speeds, '-o', 'LineWidth', 2, 'Color', [0 0.4470 0.7410], 'MarkerFaceColor', [0 0.4470 0.7410]);
xlabel('Ring Inner Radius (mm)');
ylabel('Average Flame Speed (rad/s)');
title('Flame Speed vs. Ring Radius (10 mm thickness)');
grid on;
saveas(gcf, 'flame_speed_vs_radius.png');

% Plot 2: Final state of the last simulation to visualize the collision
figure;
ax = gca;
ax.ColorOrder = [0 0 0];
pcolor(X, Y, T(:, :, end));
...
axis equal;
axis off;
title('Final Temperature Field of Colliding Flames (R_inner = 75 mm)');
c = colorbar;
c.Label.String = 'Normalized Temperature (T/T_b)';
saveas(gcf, 'colliding_flame_visualization.png');
fprintf('\nAll simulations complete. Final plots generated and saved.\n');

% --- Simple Loss Function (Function definition) ---
% =========================================================================
