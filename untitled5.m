...
% Enhanced visualization with better color schemes for combustion area
...
% Geometry
R_inner = 0.4;      % Inner radius (m)
R_outer = 0.5;      % Outer radius (m)
...
% Create custom colormap for better flame visibility
...
% Temperature evolution with enhanced visualization
subplot(2,3,1);
pcolor(X, Y, T(:,:,end));
shading interp;
colormap(gca, custom_cmap);  % Use custom colormap
colorbar;
hold on;
% Add contour lines to highlight flame front
contour(X, Y, T(:,:,end), [0.3, 0.5, 0.7], 'k-', 'LineWidth', 1.5);
hold off;
title('Temperature (Final) - Enhanced', 'FontSize', 12, 'FontWeight', 'bold');
xlabel('x (m)'); ylabel('y (m)');
axis equal tight;

% Fuel concentration with better contrast
subplot(2,3,2);
pcolor(X, Y, Y_fuel(:,:,end));
shading interp;
% Use a blue-white-red colormap for fuel (blue=consumed, white=partial, red=unburned)
fuel_cmap = [
    0.0, 0.0, 0.8;   % Blue (consumed)
    0.3, 0.3, 0.9;
    0.6, 0.6, 1.0;
    1.0, 1.0, 1.0;   % White (partially consumed)
    1.0, 0.8, 0.8;
    1.0, 0.5, 0.5;
...
colorbar;
hold on;
% Add contour to show combustion zone
...
% Combined view: Reaction Rate
subplot(2,3,3);
% Calculate reaction rate for visualization
...
shading interp;
colormap(gca, 'turbo');  % Turbo colormap is excellent for showing gradients
colorbar;
title('Reaction Rate (Combustion Zone)', 'FontSize', 12, 'FontWeight', 'bold');
xlabel('x (m)'); ylabel('y (m)');
axis equal tight;

% Flame angle over time
subplot(2,3,4);
...
% Training loss
subplot(2,3,5);
semilogy(lossHistory, 'LineWidth', 2, 'Color', [0.2, 0.4, 0.8]);
xlabel('Epoch'); ylabel('Loss');
title('Training Loss', 'FontSize', 12, 'FontWeight', 'bold');
grid on;
set(gca, 'GridAlpha', 0.3);

% Animation with enhanced colors
subplot(2,3,6);
title('Flame Propagation Animation', 'FontSize', 12, 'FontWeight', 'bold');
for n = 1:50:Nt
    pcolor(X, Y, T(:,:,n));
    shading interp;
    ...
    contour(X, Y, T(:,:,n), [0.3, 0.5, 0.7], 'w-', 'LineWidth', 1);
    hold off;
    title(sprintf('Time = %.2f s', (n-1)*dt), 'FontSize', 11);
    xlabel('x (m)'); ylabel('y (m)');
    axis equal tight;
    drawnow;
    pause(0.05);
...
%% 9. Additional Visualization - Composite View
% =========================================================================

figure('Position', [100 100 1200 500]);

% Create a composite visualization showing all aspects
subplot(1,2,1);
% Show temperature with overlay of fuel depletion
pcolor(X, Y, T(:,:,end));
shading interp;
colormap(gca, 'parula');  % Parula is MATLAB's default with good contrast
alpha(0.8);  % Make slightly transparent
colorbar;
hold on;
% Overlay fuel depletion contours
contour(X, Y, 1-Y_fuel(:,:,end), [0.3, 0.5, 0.7, 0.9], 'w-', 'LineWidth', 2);
hold off;
title('Temperature with Fuel Depletion Contours', 'FontSize', 12, 'FontWeight', 'bold');
xlabel('x (m)'); ylabel('y (m)');
axis equal tight;
...
surf(X, Y, T(:,:,end));
shading interp;
colormap(gca, 'jet');  % Jet provides high contrast
colorbar;
view(45, 30);
lighting gouraud;
light('Position', [1 1 1]);
title('3D Temperature Surface', 'FontSize', 12, 'FontWeight', 'bold');
...
fprintf('Average Flame Speed: %.3f rad/s\n', flame_speed);
...
% Identify peak combustion zone
[max_omega, max_idx] = max(omega_vis(:));
[i_max, j_max] = ind2sub([Nr, Ntheta], max_idx);
fprintf('Peak Reaction Rate: %.4f at r=%.3f m, θ=%.1f°\n', ...
    max_omega, r(i_max), theta(j_max)*180/pi);

fprintf('\nSimulation complete with enhanced visualization!\n');