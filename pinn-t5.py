...
class TJunctionTrainer:
    ...
    def generate_tjunction_points(self, n_points=100000):
        ...
        # MODIFIED: Extended time range for training
        t_main = torch.rand(n_main, 1) * 1.5
        ...
        # MODIFIED: Extended time range for training
        t_branch = torch.rand(n_branch, 1) * 1.5
        ...
        # MODIFIED: Extended time range for training
        t_junction = torch.rand(n_junction, 1) * 1.5
    ...
    def generate_boundary_points(self, n_points=1000):
        ...
        # MODIFIED: Extended time range for training
        t_inlet = torch.rand(n_inlet, 1) * 1.5
        ...
        # MODIFIED: Extended time range for training
        t_outlet = torch.rand(n_outlet, 1) * 1.5
        ...
        # MODIFIED: Extended time range for training
        t_branch_out = torch.rand(n_branch_out, 1) * 1.5
    ...
    def train(self, epochs=4000, print_freq=100):
    ...
    def visualize_results(self, time_points=[0.0, 0.3, 0.6, 0.9]):
...
def main():
    ...
    # MODIFIED: Train model with more epochs for better convergence over a longer time
    trainer.train(epochs=4000, print_freq=400)
    
    # MODIFIED: Visualize results at extended time points for natural propagation
    trainer.visualize_results(time_points=[0.0, 0.3, 0.6, 0.9])
