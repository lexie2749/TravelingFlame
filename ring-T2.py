...
class TJunctionCombustionPINN(nn.Module):
    ...
    def __init__(self, layers, activation=torch.tanh):
        ...
        self.Re = 80.0     # Lower Reynolds for better flow splitting
        self.Pe = 40.0     # Lower Peclet for better heat diffusion
        self.Sc = 0.8      # Lower Schmidt for better species mixing
        self.Da = 12.0     # Higher Damkohler for stronger reaction
        self.beta = 4.0    # Higher heat release
    ...
    def forward(self, x, y, t):
        """Forward pass with geometry-aware constraints and flow splitting"""
        ...
        # Enhanced velocity field to encourage splitting
        u_base = torch.tanh(outputs[:, 0:1]) * 3.0
        v_base = torch.tanh(outputs[:, 1:2]) * 2.0
        
        # Add flow splitting near junction (x=1.0, y=1.0)
        junction_factor = torch.exp(-((x - 1.0)**2 / 0.5 + (y - 1.0)**2 / 0.5))
        v_split = v_base + junction_factor * 1.5  # Add upward component at junction
    ...
    def reaction_rate(self, T, Y_fuel):
        """Enhanced reaction rate for T-junction with better splitting"""
        T_safe = torch.clamp(T, min=0.1, max=10.0)
        Y_safe = torch.clamp(Y_fuel, min=1e-6, max=1.0)
        
        # Lower activation energy for easier propagation
        activation_term = -1.0 / T_safe
        activation_term = torch.clamp(activation_term, min=-10, max=10)
        
        rate = self.Da * Y_safe * torch.exp(activation_term)
        
        # Lower ignition temperature for better branching
        ignition_factor = torch.sigmoid((T_safe - 0.5) * 20.0)
        
        final_rate = rate * ignition_factor
        return torch.clamp(final_rate, min=0.0, max=150.0)
...
class TJunctionTrainer:
    ...
    def generate_tjunction_points(self, n_points=5000):
        ...
        t_junction = torch.rand(n_junction, 1) * 0.2
    ...
    def train_step(self, x_int, y_int, t_int, boundary_data):
        ...
        # Add vertical velocity component near junction level (y around 0.5-1.0)
        v_target = torch.where((y_in > 0.3) & (y_in < 0.8), 
                              0.5 * torch.ones_like(v_in),  # Upward flow near junction level
                              torch.zeros_like(v_in))
        
        loss_inlet = torch.mean((u_in - 3.0)**2) + torch.mean((v_in - v_target)**2) + \
                    torch.mean((T_in - 3.0)**2) + torch.mean((Y_in - 0.95)**2)
        
        # 3. Initial condition with hot spot at junction (x=1.0, y=1.0)
        x_ic, y_ic, t_ic = boundary_data['initial']
        u_ic, v_ic, _, T_ic, Y_ic = self.model(x_ic, y_ic, t_ic)
        
        # Place hot spot at junction (x=1.0, y=0.75)
        T_init = 0.3 + 3.5 * torch.exp(-((x_ic - 1.0)**2 / 0.4 + (y_ic - 0.75)**2 / 0.3))
        # Add secondary hot spot to encourage branching at y=1.0
        T_init += 2.5 * torch.exp(-((x_ic - 1.0)**2 / 0.2 + (y_ic - 1.0)**2 / 0.15))
        
        Y_init = 0.9 * torch.ones_like(x_ic)
        
        # Initial velocity field to encourage splitting at x=1.0
        u_init = 2.0 * torch.ones_like(u_ic)
        v_init = torch.where((x_ic > 0.5) & (x_ic < 1.5) & (y_ic > 0.5) & (y_ic < 1.2),
                            1.0 * torch.ones_like(v_ic),  # Strong upward flow at junction
    ...
    def train(self, epochs=2500, print_freq=100):
    ...
    def visualize_results(self, time_points=[0.0, 0.04, 0.08, 0.12]):
        """Visualize T-junction flame propagation with enhanced detail"""
        ...
        plt.suptitle('T-Junction Flame Splitting (Branch at x=1.0)', fontsize=16, fontweight='bold')
        plt.tight_layout()
        plt.show()
...
def main():
    ...
    trainer.train(epochs=2500, print_freq=250)
    
    # Visualize results at more time points to see splitting
    trainer.visualize_results(time_points=[0.0, 0.04, 0.08, 0.12])
