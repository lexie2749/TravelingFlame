...
if relevant_experiments:
    ...
    plt.ylabel('ω (Angular Velocity)', fontsize=12)
    plt.legend(title="Experiment", loc='best', fontsize=9)
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.tick_params(axis='both', which='major', labelsize=10)
    plt.tight_layout()
...
if len(experiment_dataframes) > 0:
    ...
    for idx, exp in enumerate(experiment_dataframes):
        df_exp = exp['df']
        if not df_exp.empty:
            plt.scatter(df_exp['t'], df_exp['omega'], 
                       s=15, alpha=0.6, color=colors[idx], 
                       label=exp['info'])
    
    plt.title('All Experiments - Positive Values Only', fontsize=16)
    plt.xlabel('t (Time)', fontsize=14)
    plt.ylabel('|ω| (Absolute Angular Velocity)', fontsize=14)
